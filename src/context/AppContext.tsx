...
export const AppProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const [inventory, setInventory] = useState<PharmacyInventoryItem[]>(MOCK_INVENTORY);
  const [orders, setOrders] = useState<OrderRequest[]>(MOCK_ORDERS);
  const [feedbacks, setFeedbacks] = useState<FeedbackRecord[]>(MOCK_FEEDBACKS);
  const [auditLogs, setAuditLogs] = useState<AuditLog[]>(MOCK_AUDIT_LOGS);
  const [verificationApps, setVerificationApps] = useState<PharmacyVerificationApplication[]>(MOCK_VERIFICATION_APPLICATIONS);
  const [customerProfile] = useState<CustomerProfile>(MOCK_CUSTOMER_PROFILE);
  const [pharmacyStaff, setPharmacyStaff] = useState<PharmacyStaff[]>(MOCK_PHARMACY_STAFF);
  ...
  const currentSellerPharmacy = pharmacies.find(p => p.id === currentSellerPharmacyId) || pharmacies[0];
  ...
  const findGenericMatches = (targetMed: Medicine): GenericAlternativeMatch[] => {
    // Normalizes active salt matching
    const primarySalt = targetMed.activeSalt.toLowerCase().split('(')[0].trim();
    
    // Find all medicines that share the primary active salt
    ...
    return matches.map(alt => {
      // Calculate savings relative to target
  ...
  const createOrderRequest = (orderData: {
    ...
    const newLog: AuditLog = {
      ...
      actorRole: 'Customer',
      actorName: customerProfile.fullName,
    ...
    setAuditLogs(prev => [
      {
        ...
        timestamp: new Date().toISOString(),
        actorRole: 'Pharmacy',
        actorName: currentSellerPharmacy.mspcPharmacistName,
  ...
  const submitFeedback = (fbData: Omit<FeedbackRecord, 'id' | 'timestamp'>) => {
    ...
    // If discrepancy reported, log audit warning
  ...
  const updateInventoryItem = (itemId: string, newStock: number, newPrice: number) => {
    ...
    // Update pharmacy freshness
    setPharmacies(prev => prev.map(p => {
      if (p.id === currentSellerPharmacyId) {
        return {
          ...p,
          lastStockSyncTimestamp: 'Synced 1 min ago',
          stockFreshnessMinutes: 1
        };
      }
      return p;
    }));
  ...
  const approvePharmacyVerification = (appId: string, notes?: string) => {
    ...
    if (app) {
      // Add or update pharmacy verified badge
