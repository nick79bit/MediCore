...
export const LandingPage: React.FC<LandingPageProps> = ({ onEnter }) => {
  ...
  return (
    <div className="min-h-screen flex flex-col" style={{ fontFamily: 'var(--font-sans)' }}>
      ...
      {/* ── Hero ── */}
      <section
        className="relative overflow-hidden flex-1 flex flex-col items-center justify-center px-4 py-20 sm:py-28 text-center"
        style={{
      ...
      >
        ...
        />
        <div
          className="absolute bottom-20 left-8 w-56 h-56 rounded-full opacity-15 blur-3xl pointer-events-none"
        ...
        >
          ...
          >
            Save Up to{' '}
            <span className="text-gradient">62%</span>{' '}
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-3">
            <button
              onClick={() => onEnter('customer')}
              className="btn btn-primary text-sm px-6 py-3 gap-2 shadow-lg"
              style={{ boxShadow: '0 6px 24px -6px rgba(13,148,136,0.5)' }}
            >
        ...
        <div className="max-w-5xl mx-auto">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 stagger">
            {FEATURES.map((f) => (
              <div
                key={f.title}
                className="card p-6 animate-fade-in group"
              >
            ...
            {ROLES.map((r) => (
              <button
                ...
                style={{
                  ...
                  boxShadow: hoveredRole === r.role ? `0 8px 40px -8px ${r.glow}, 0 2px 8px rgba(0,0,0,0.06)` : '0 2px 8px rgba(0,0,0,0.04)',
                  transform: hoveredRole === r.role ? 'translateY(-4px)' : 'translateY(0)',
                }}
              >
                {/* Gradient overlay on hover */}
                <div
                  className={`absolute inset-0 bg-gradient-to-br ${r.gradient} opacity-0 group-hover:opacity-5 transition-opacity duration-300 rounded-2xl pointer-events-none`}
                />
